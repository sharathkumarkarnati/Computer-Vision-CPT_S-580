...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.zeros(N, C, H1, W1, dtype = x.dtype, device = x.device)
          ...
          for c in range(C):
            for wi in range(W1):
              for hi in range(H1):
                x_start = wi * stride 
                y_start = hi * stride
                out[n, c, hi, wi] = torch.max(x[n, c, y_start:y_start + pool_height, x_start:x_start + pool_width])
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['b1'] = torch.zeros(F, device=device, dtype = self.dtype)
        
        self.params['W2'] = weight_scale * torch.randn(F * HP * WP, hidden_dim,
                                                      device=device, dtype = self.dtype)
        self.params['b2'] = torch.zeros(hidden_dim, device=device, dtype = self.dtype)
        
        self.params['W3'] = weight_scale * torch.randn(hidden_dim, num_classes,
                                                      device=device, dtype = self.dtype)
        self.params['b3'] = torch.zeros(num_classes, device=device, dtype = self.dtype)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def loss(self, X, y=None):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = X.shape
        X2, Conv_ReLU_Pool_cache = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        X3, Linear_ReLU_cache = Linear_ReLU.forward(X2, W2, b2)
        scores, linear_forward_cache = Linear.forward(X3, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dx3, dw3, db3 = Linear.backward(dout, linear_forward_cache)
        ...
        dx1, dw1, db1 = Conv_ReLU_Pool.backward(dx2, Conv_ReLU_Pool_cache)
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = math.sqrt(gain / Din) * torch.randn(Din, Dout, device=device, dtype = dtype)
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            sample_var = torch.sum(sq, axis = 0) / (N - 1)
            var = sample_var
    ...
    def backward(dout, cache):
        ...
        dvar = 0.5 * dsqrtvar / torch.sqrt(var + eps)
        dsq = torch.ones_like(dout) * dvar / (N - 1)
        dxmu2 = 2 * xmu * dsq
        dx1 = dxmu1 + dxmu2
        dmu = - torch.sum(dx1, dim = 0)
        dx2 = torch.ones_like(dout) * dmu / N
        dx = dx1 + dx2
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        xhat, gamma, xmu, ivar, sqrtvar, var, eps = cache
        dgamma = torch.sum(dout * xhat, dim = 0)
        dx = gamma * ((dout - torch.sum(dout, dim = 0) / N) / sqrtvar
                      - xmu * torch.sum(dout * xmu, dim = 0) / (sqrtvar**3 * (N - 1)))
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = x.shape
        x1 = x.contiguous().transpose(0, 1).contiguous().view(C, N * H * W).transpose(0, 1)
    ...
    def backward(dout, cache):
        ...
        dx1, dgamma, dbeta = BatchNorm.backward_alt(dout1, cache)
