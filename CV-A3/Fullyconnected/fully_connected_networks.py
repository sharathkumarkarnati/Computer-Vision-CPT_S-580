...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
    ...
    self.params['W1'] = weight_scale * torch.randn(D, H, device=device).to(dtype)
    self.params['W2'] = weight_scale * torch.randn(H, C, device=device).to(dtype)
    self.params['b1'] = torch.zeros(H, device=device).to(dtype)
    self.params['b2'] = torch.zeros(C, device=device).to(dtype)
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
  ...
  def loss(self, X, y=None):
    ...
    b1 = self.params['b1']
    b2 = self.params['b2']
    X2, Linear_ReLU_cache = Linear_ReLU.forward(X, W1, b1)
    scores, Linear.forward_cache = Linear.forward(X2, W2, b2)
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    dx2, dw2, db2 = Linear.backward(dout, Linear.forward_cache)
    grads['W2'] = dw2 + 2 * self.reg * W2
    grads['b2'] = db2
    dx1, dw1, db1 = Linear_ReLU.backward(dx2, Linear_ReLU_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(len(dims) - 1):
      self.params['W' + str(i + 1)] = weight_scale * torch.randn(dims[i], dims[i + 1], device=device).to(dtype)
      self.params['b' + str(i + 1)] = torch.zeros(dims[i + 1], device=device).to(dtype)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1):
      param_num = str(i + 1)
      W = self.params['W' + param_num]
      b = self.params['b' + param_num]
      X_next, cache = Linear_ReLU.forward(X_next, W, b)
      linear_Relu_cache.append(cache)
      if self.use_dropout:
        X_next, do_cache = Dropout.forward(X_next, self.dropout_param)
        dropout_cache.append(do_cache)

    W = self.params['W' + str(self.num_layers)]
    b = self.params['b' + str(self.num_layers)]
    scores, Linear.forward_cache = Linear.forward(X_next, W, b)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dx, dw, db = Linear.backward(dout, Linear.forward_cache)
    param_num = str(self.num_layers)
    grads['W' + param_num] = dw + self.reg * self.params['W' + param_num]
    grads['b' + param_num] = db

    for i in range(self.num_layers - 2, -1, -1):
      if self.use_dropout:
        dx = Dropout.backward(dx, dropout_cache[i])
      dx, dw, db = Linear_ReLU.backward(dx, linear_Relu_cache[i])
      param_num = str(i + 1)
      grads['W' + param_num] = dw + self.reg * self.params['W' + param_num]
      grads['b' + param_num] = db
    ############################################################################
