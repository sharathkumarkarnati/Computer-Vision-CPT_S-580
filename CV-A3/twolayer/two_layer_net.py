...
def nn_forward_backward(
    ...
    dh1 = temp.mm(W2.T) / N  # The same as dW2
    dh1[h1 == 0] = 0
    dW1 = X.T.mm(dh1)
    dW1 += 2 * reg * W1
    db1 = dh1.sum(dim=0)
        ...
        for regularization_strength in regularization_strengths:
            for hidden_size in hidden_sizes:
                for learning_rate_decay in learning_rate_decays:
                    net = TwoLayerNet(D, hidden_size, C)
                    stat = net.train(
                        data_dict["X_train"],
                        data_dict["y_train"],
                        data_dict["X_val"],
                    ...
                    print(
                    "Learning rate: %e | Regularization strength: %e | Hidden size: %d | "
                    "Training accuracy: %f | Validation accuracy: %f"
                    % (
                      learning_rate,
                      regularization_strength,
                      hidden_size,
                      stat["train_acc_history"][-1],
                      stat["val_acc_history"][-1],
                      )
                      )
