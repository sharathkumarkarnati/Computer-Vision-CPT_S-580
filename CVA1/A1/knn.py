...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test = x_test.reshape(num_test, -1)
    x_train = x_train.reshape(num_train, -1)
    dists = torch.sum(x_train**2, dim=1).reshape(-1, 1)+torch.sum(x_test **
                                                                  2, dim=1).reshape(1, -1)-2*torch.mm(x_train, x_test.t())
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
import cv2
import numpy as np
def compute_distances_N(x_train: torch.Tensor, x_test: torch.Tensor):
    # Initialize dists to be a tensor of shape (num_train, num_test) with the
    # same datatype and device as x_train

    sift = cv2.SIFT_create()
    img1 = (x_train.detach().numpy()*255).astype(np.uint8)
    img1 = np.transpose(img1, (0, 2, 3, 1))
    print("Input image in numpy ", img1.shape, np.max(img1))
    kp1, des1 = sift.detectAndCompute(img1[0], None)
    kp2, des2 = sift.detectAndCompute(img1[1], None)
    ...
    print("Output shape ", len(kp1), kp1[0].pt, des1.shape)
    num_train = x_train.shape[0]
    num_test = x_test.shape[0]
    dists = x_train.new_zeros(num_train, num_test)
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ...
        values, indices = y_train[indices].unique(return_counts=True)
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
          ...
          fx_train = torch.cat([x_train_folds[j]
                                  for j in range(num_folds) if i != j])
